...
-- 针对当前的数据库实例是有效的，一旦重启mysql服务，就失效了。

-- 会话系统变量：
-- 方式1：
SET
@@session.character_set_client = 'gbk';

SET
SESSION character_set_client = 'gbk';

-- 针对当前会话是有效的，一旦结束会话，重新建立起新的会话，就失效了
...
1、局部变量必须
① 使用DECALRE声明
② 声明并使用BEGIN ... END 中（使用在存储过程、函数中）
③ DECLARE的方式声明局部变量必须声明在BEGIN中的首行的位置
...
-- 举例1：声明局部变量，并分别赋值为employees 表中employee_id为102的last_name和salary
delimiter //
...
BEGIN 
	-- 分析：查询出emp_id员工的工资；查询出emp_id员工的管理者的id；查询管理者id的工资；计算两个工资的插值
	-- 声明变量
	DECLARE emp_sal double DEFAULT 0.0; -- 记录员工的工资
	DECLARE mgt_sal double DEFAULT 0.0; -- 记录管理者的工资
	...
	SELECT salary INTO mgt_sal FROM employees WHERE employee_id = mgr_id;

	SET dif_salary = mgt_sal - emp_sal;
END //
delimiter ;
...
INSERT INTO employees(last_name) VALUES ('Tom');

DESC employees;

-- 错误演示：
delimiter //
CREATE PROCEDURE UpdateDataCondition()
BEGIN 
	SET @x = 1;
...
-- 2.4 案例的处理
DROP PROCEDURE UpdateDataCondition;
...
BEGIN
    -- 处理程序
	-- 方式1：
	-- DECLARE EXIT handler FOR 1062 @pro_value = -1;
	-- 方式2：
...
-- 判断该员工薪资如果低于9000元并且入职时间超过5粘，就涨薪500元；否则就涨薪100元
delimiter //
...
-- 调用
CALL update_salary_by_eid4(:emp_id);
...
DROP PROCEDURE update_salary_by_eid5(INT);
...
-- 存储过程中实现循环给大家降薪，薪资降为原来的90%。直到全公司的平均薪资达到5000结束。
-- 并统计循环次数。
delimiter //
...
BEGIN
	-- 
	DECLARE avg_sal; -- 记录平均工资
	DECLARE while_count int DEFAULT 0; -- 记录循环次数
	
	SELECT avg(salary) FROM employees; -- ① 初始化条件
...
BEGIN
	...
	SELECT avg(salary) INTO avg_sal FROM employees e JOIN departments d ON e.department_id = d.department_id WHERE d.department_name = dept_name;
